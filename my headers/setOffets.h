...
DWORD sub = 0x10; //subs from offset to make a bigger search zone
...
bool stablishOffsets() {
	...
	//===============|+MODULOS|=================================
	// + client.dll
	offset::EntityList = findpattern(true, true, (0x402E2D - sub),offsetload::offset[0], 2, 0x10, clientmodule, "\x8B\x88\x00\x00\x00\x00\x85\xC9\x74\x42\x8B\x01\xFF\x50\x00\x8B\xF8\x85\xFF\x74\x37\x8B\x17\x8B\xCF\x8B\x92\x00\x00\x00\x00\xFF\xD2\x84\xC0\x74\x27", "xx????xxxxxxxx?xxxxxxxxxxxx????xxxxxx");
	if (offset::EntityList == errorCode) { return false; }

	offset::PlayerBase = findpattern(true, true, (0x2ACAF9 - sub), offsetload::offset[1], 3, 4, clientmodule, "\x8D\x34\x85\x00\x00\x00\x00\x89\x15\x00\x00\x00\x00\x8B\x41\x08\x8B\x48\x04\x83\xF9\xFF", "xxx????xx????xxxxxxxxx");
	...
	offset::ForceShoot = findpattern(true, true, (0x2ED5D5 - sub), offsetload::offset[3], 2, 0, clientmodule, "\x8B\x0D\x00\x00\x00\x00\x8B\xD6\x8B\xC1\x83\xCA\x01\x24\x03", "xx????xxxxxxxxx");
	if (base::isMenuOpenBase == errorCode) { return false; }

	base::isMenuOpenBase = findpattern(false, true, (0x47B448-sub), offsetload::offset[4], 1, 0, clientmodule, "\xA1\x00\x00\x00\x00\x32\xD2\xB9\x00\x00\x00\x00\x89\x55\xF8\x8B\x40\x74\xFF\xD0", "x????xxx????xxxxxxxx");
	if (base::isMenuOpenBase == errorCode) { return false; }

	// + engine.dll
	offset::ClientState = findpattern(true,true, (0xB711B-sub), offsetload::offset[5], 1, 0, enginemodule, "\xA1\x00\x00\x00\x00\x33\xD2\x6A\x00\x6A\x00\x33\xC9\x89\xB0", "x????xxxxxxxxxx");
	...
	// + ClientState
	offset::dwClientState_GetLocalPlayer = findpattern(false, true, (0xE020B-sub), offsetload::offset[12], 2, 0, enginemodule, "\x8B\x80\x00\x00\x00\x00\x40\x5D\xC2\x04\x00\x83\xC8\xFF\x5D", "xx????xxxxxxxxx");
